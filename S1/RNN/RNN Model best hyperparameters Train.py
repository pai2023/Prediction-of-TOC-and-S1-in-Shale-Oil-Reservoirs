...
X = data.iloc[:, 1:-1].values  # From the second column to the second-to-last column
y = data.iloc[:, -1].values  # Label column remains unchanged
...
class RNNRegression(nn.Module):
    ...
    def forward(self, x):
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(x.device)  # Initialize hidden layer state to GPU/CPU
        out, _ = self.rnn(x, h0)
        out = self.fc(out[:, -1, :])
        return out
            ...
            for train_index, val_index in kf.split(dataset):
                ...
                for epoch in range(100):
                    for features, labels in train_loader:
                        features, labels = features.to(device), labels.to(device)  # Move features and labels to GPU/CPU
                        optimizer.zero_grad()
