...
# **2️⃣ Read data**
labeled_data = pd.read_excel(r"C:\Users\User\Desktop\test\calculated_curves\S1\training_and_validation_data-S1.xlsx",
                             sheet_name="Training_Set")
unlabeled_data = pd.read_excel(r"C:\Users\User\Desktop\test\calculated_curves\S1\training_and_validation_data-S1.xlsx",
...
best_r2_supervised = -np.inf # Used for comprehensive scoring
...
def train_model(num_epochs, model, train_dataloader, X_train, y_train, X_test, y_test):
    ...
    # 👉 Only mask data labeled as “supervised”
    num_labeled = len(y_labeled)  # Initial number of labeled data points
    supervised_mask = torch.zeros_like(y_train, dtype=torch.bool)
    supervised_mask[:num_labeled] = True  # The first num_labeled is labeled.
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            # ✨ New: Calculate R² using only supervised data
...
for iteration in range(self_training_iterations):
    ...
    if best_model_overall is not None:
        model.load_state_dict(best_model_overall.state_dict())  # Reload the best model

    # Retrain the model
    train_model(num_epochs, model, train_dataloader, X_train, y_train, X_test, y_test)

# 9️⃣ Save the best model
torch.save(best_model_test.state_dict(), 'best_test_model-S1.pth')
torch.save(best_model_train.state_dict(), 'best_train_model-S1.pth')
torch.save(best_model_overall.state_dict(), 'best_overall_model-S1.pth')
