...
X = torch.load("X_train_well_log_data.pt")
Y = torch.load("Y_train_labels.pt")
...
class CNNRegression(nn.Module):
    def __init__(self, input_size, conv_filters, kernel_size, num_conv_layers, output_size):
        ...
        for i in range(num_conv_layers):
            if i == 0:
                layers.append(nn.Conv1d(1, conv_filters, kernel_size, padding='same'))
            else:
